...
/// The overlay's content: ask box + the brain's reply. Renders the verdict the
/// brain returns verbatim — a cited answer, or the honest "No one wrote this down."
/// It never decides grounding itself. Styling is intentionally plain here; the
...
struct OverlayView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            case .response(let r) where r.verdict == .answer:
                answer(r)
            case .response(let r):
                honestUnknown(r)   // verdict == .unknown
            case .unreachable:
    ...
    private func chip(_ citation: Citation) -> some View {
        ...
        if let urlString = citation.url, let url = URL(string: urlString) {
            ...
            label.foregroundStyle(.secondary)   // ref with no resolvable URL
        }
    }
